...
task.loadNpmTasks = function(name) {
  var dirs = [path.resolve('node_modules')].concat(task.npmDirs);
  dirs.forEach(function(root) {
    loadNpmTasks(root, name);
  });
};

function loadNpmTasks(root, name) {
  ...
  if (pkg.keywords && pkg.keywords.indexOf('gruntcollection') !== -1) {
    Object.keys(pkg.dependencies).forEach(function(depName) {
      ...
      if (filepath) {
        ...
        loadNpmTasks(path.relative(root, filepath));
      }
    });
    return;
  }
